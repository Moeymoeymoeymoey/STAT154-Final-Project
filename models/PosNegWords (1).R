...
mcterms.train = mcterms[-rs, ]
mcterms.test = mcterms[rs, ]
business.id.train = yelp$business_id[-rs]
business.id.test = yelp$business_id[rs]
stars.train = yelp$stars[-rs]
stars.test = yelp$stars[rs]
...
negWords = names(sort(posColsumstrain - negColsumstrain, decreasing = F))[1:50]
posWords = names(sort(posColsumstrain - negColsumstrain, decreasing = T))[1:50]
